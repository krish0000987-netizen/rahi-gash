...
import { Phone, MapPin, Clock, ArrowRight, MessageSquare, ShieldCheck, Award, Sparkles } from 'lucide-react';
import { BUSINESS_INFO } from '../data/siteData';

export default function Footer({ onOpenQuoteModal }) {
  return (
    <footer className="bg-black text-gray-400 pt-16 pb-24 lg:pb-12 border-t border-neutral-900 relative overflow-hidden">
      {/* Subtle Background Glow */}
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-96 h-96 bg-red-950/20 blur-[120px] pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="mb-14 p-8 rounded-3xl bg-gradient-to-r from-neutral-900/90 via-neutral-900 to-red-950/40 border border-red-900/30 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl">
          <div>
            <span className="text-red-500 font-semibold text-xs tracking-widest uppercase">Transform Your Business Visibility</span>
            <h3 className="text-2xl sm:text-3xl font-extrabold text-white mt-1">Need LED Boards, Signage or Flex Printing?</h3>
            <p className="text-gray-400 text-sm mt-1">Get custom design previews, transparent pricing, and fast local Ghaziabad delivery.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 pb-12 border-b border-neutral-900">
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-gray-500 tracking-wider">GHAZIABAD STUDIO</p>
              </div>
            </div>
            ...
            <div className="space-y-2.5 pt-2 text-xs">
              ...
              <div className="flex items-center gap-3">
                ...
                <a href={`tel:${BUSINESS_INFO.phone}`} className="text-white hover:text-red-400 font-medium">
                  {BUSINESS_INFO.phone}
                </a>
              </div>
              <div className="flex items-center gap-3">
                <Clock className="w-4 h-4 text-red-500 shrink-0" />
                <span className="text-gray-400">{BUSINESS_INFO.workingHours}</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-xs">
              <li><Link to="/led-boards" className="hover:text-white transition-colors">LED Boards</Link></li>
              <li><Link to="/flex-printing" className="hover:text-white transition-colors">Flex Banner Printing</Link></li>
              <li><Link to="/signage-3d" className="hover:text-white transition-colors">3D LED Lettering</Link></li>
              <li><Link to="/signage-3d" className="hover:text-white transition-colors">Shop Sign Boards</Link></li>
              <li><Link to="/printing" className="hover:text-white transition-colors">Visiting Card Printing</Link></li>
              <li><Link to="/printing" className="hover:text-white transition-colors">Wedding Card Printing</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {['Khoda Colony', 'Indirapuram', 'Vaishali', 'Vasundhara', 'Noida Sec 62', 'Ghaziabad City'].map((area) => (
                <span key={area} className="px-2 py-1 rounded bg-neutral-900 text-[10px] text-gray-300 border border-neutral-800">
                  {area}
                </span>
              ))}
            </div>
